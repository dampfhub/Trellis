...
string
Util::PathBaseName(string const &path) {
    return path.substr(path.find_last_of("/\\") + 1);
}

bool
Util::IsPng(string file) {
    return (file.substr(file.find_last_of(".") + 1) == "png");
}

template<>
string
Util::deserialize<string>(const vector<byte> &bytes) {
...
template<>
vector<byte>
Util::serialize_vec<string>(const string &object) {
    vector<byte> bytes;
...
vector<byte>
Util::serialize(const string &str) {
    size_t       s     = str.size() + 1;
...
vector<byte>
Util::flatten(vector<vector<byte>> vecs) {
    vector<byte> bytes;
    for (auto &v : vecs) { bytes.insert(bytes.end(), v.begin(), v.end()); }
    return bytes;
}
...
size_t
Util::hash_image(vector<unsigned char> const &vec) {
