...
glm::mat4 Camera2D::CalculateView(glm::vec2 board_dims) {
    ...
    // { { p->d - m / z } }
    if ((board_dims.x * ZoomFactor - Position.x) <
            ((float)glfw.GetScreenWidth() - Bounds)) {
        Position.x = board_dims.x * ZoomFactor -
                ((float)glfw.GetScreenWidth() - Bounds);
    }
    if ((board_dims.y * ZoomFactor - Position.y) <
            ((float)glfw.GetScreenHeight() - Bounds)) {
        Position.y = board_dims.y * ZoomFactor -
                ((float)glfw.GetScreenHeight() - Bounds);
    }
    ...
    view = glm::scale(
            view, glm::vec3(ZoomFactor, ZoomFactor, 1.0f));
    return view;
}